  ...
  const downloadOBJ = useCallback(() => {
    ...
    setTimeout(async () => {
      try {
        ...
        mtlContent += `# Generated by zhizao3d - ${new Date().toISOString()}\n`;
  ...
  const downloadWithTextures = useCallback(() => {
    ...
    setTimeout(async () => {
      try {
        ...
        let errorDescription = '纹理提取失败。';

        if (error instanceof Error) {
          if (error.message.includes('No PBR textures found')) {
            errorDescription += '该模型不包含任何 PBR 纹理贴图。';
          } else if (error.message.includes('Failed to convert')) {
            errorDescription += '纹理转换失败，可能不支持该纹理格式。';
          } else if (error.message.includes('WebGL')) {
            errorDescription += '图形渲染出现问题，请尝试刷新页面。';
          }
        } else {
          errorDescription += '纹理提取过程中发生未知错误。';
        }

        toast({
          title: '纹理提取失败',
          description: errorDescription + '你仍可下载标准 GLB 文件。',
          variant: 'destructive',
          duration: 8000,
        });
      } finally {
  ...
  const sendToMandarin3D = useCallback(() => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const { id } = toast({
          title: '正在准备模型',
          description: '正在为打印准备你的 3D 模型，可能需要一点时间。',
        });
        preparingToastId = id;
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.message || '发送模型到 Mandarin3D 失败');
        }
        ...
        if (mandarinWindow) {
          ...
          toast({
            title: '成功！',
            description: '模型已发送至 Mandarin3D，即将跳转完成订单。',
            duration: 3000,
          });
        } else {
          // Popup was blocked
          toast({
            ...
            description: (
              <div className="flex flex-col gap-2">
                <p>请允许本站弹窗，或点击下方按钮打开 Mandarin3D。</p>
                <Button
  ...
  const downloadGLB = useCallback(() => {
    ...
    setTimeout(async () => {
      try {
        ...
        toast({
          title: '错误',
          description: '增强版 GLB 生成失败，将改为下载原始文件。',
          variant: 'destructive',
        });
          ...
          {Object.values(hasPBRMaps).some(Boolean) && (
            <>
              ...
              >
                ...
                <span className="text-sm">.ZIP 含贴图</span>
                <span className="ml-3 text-xs text-adam-text-primary/60">
                  {isDownloadingWithTextures ? '下载中…' : '含贴图的 GLB'}
                </span>
              </DropdownMenuItem>
            </>
          )}
