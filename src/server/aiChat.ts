...
// "Relay mode": OPENROUTER_BASE_URL pointing at a generic OpenAI-compatible
// relay routes EVERY catalog model through it, via the OpenAI *Responses*
// API rather than chat/completions — tested relays (grsai) proxy Responses
// upstreams and silently drop tool_calls on the chat endpoint, which breaks
// the tool-driven parametric flow. Relays register bare model names
// ("gpt-5.6-sol"), not vendor-prefixed ids ("openai/gpt-5.6-sol").
function isCustomRelay(): boolean {
  return Boolean(env('OPENROUTER_BASE_URL').trim());
}

function relayModelId(modelId: string): string {
...
function createChatProviders(): ChatProviders {
  ...
  let relay: ReturnType<typeof createOpenAI> | undefined;
  return {
    relay: () => {
      relay ??= createOpenAI({
...
// Relay mode goes through the OpenAI Responses API, which takes the flat
// `tool_choice: { type: "function", name }` shape the AI SDK's OpenAI
// provider emits natively — forcing works there, so no relay exception.
